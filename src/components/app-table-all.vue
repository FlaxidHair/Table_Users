  ...
  <div>
    <v-container fluid style="width:85vw;">
        ...
        <v-row justify="end">
          <v-col class="d-flex" sm="2">
            <v-select filled label="Действия" v-model="$store.state.actionsSelect" :items="$store.state.itemChoose"
              background-color="blue" color="blue" style="height:50px;">
      ...
      <div v-if="$store.state.isFind">
        <v-data-table no-data-text="Нет результатов" :headers="$store.state.itemsInnerStatus"
          :items="$store.getters.getUserFinded" :items-per-page="5" class="elevation-1" v-model="$store.state.selected"
      ...
      <div v-else-if="!$store.state.isFind">
        <v-data-table :headers="$store.state.itemsInnerStatus" :items="$store.getters.getUserss" :items-per-page="5"
